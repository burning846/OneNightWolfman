...
function reducer(state, a) {
  switch (a.type) {
    case 'set_status': return { ...state, status: a.value };
    case 'set_socket_connected': return { ...state, socketConnected: a.value };
    case 'set_user': return { ...state, user: a.user };
    case 'set_room':
      return { ...state, status: 'in_room', roomCode: a.roomCode, playerId: a.playerId, token: a.token };
    case 'reset':
      return { ...initial, status: 'home', user: state.user };
    case 'room_state': {
      const players = a.payload.players;
      const myIdx = state.playerId ? players.findIndex(p => p.id === state.playerId) : -1;
      const reset = a.payload.phase === 'lobby'
        ? {
            myRole: null, nightStep: null, nightPrompt: null, pendingReveal: null,
            nightReveals: [], dayEndsAt: null, voteEndsAt: null, stepEndsAt: null,
            voteProgress: { count: 0, total: 0 }, myVote: null, result: null,
          }
        : {};
      const phaseSync = {};
      ...
      if (a.payload.stepEndsAt != null) phaseSync.stepEndsAt = a.payload.stepEndsAt;
    ...
    case 'night_step':
      return {
        ...
        nightPrompt: null,
        pendingReveal: null,
        stepEndsAt: a.payload.endsAt ?? state.stepEndsAt,
      };
    case 'night_prompt': return { ...state, nightPrompt: a.payload };
    case 'night_reveal':
      return { ...state, pendingReveal: a.payload, nightReveals: [...state.nightReveals, a.payload] };
    case 'ack_reveal': return { ...state, pendingReveal: null, nightPrompt: null };
    case 'day_phase': return { ...state, dayEndsAt: a.payload.endsAt };
    case 'vote_phase': return { ...state, voteEndsAt: a.payload.endsAt, myVote: null };
    case 'vote_progress': return { ...state, voteProgress: a.payload };
    case 'set_my_vote': return { ...state, myVote: a.value };
    case 'result': return { ...state, result: a.payload };
    case 'set_error': return { ...state, errorMessage: a.message };
    case 'clear_error': return { ...state, errorMessage: null };
...
export function GameProvider({ children }) {
  ...
  const api = useMemo(() => ({
    ...
    createRoom: (nickname) => new Promise((resolve) => {
      const sock = ensureSocket();
    ...
    joinRoom: (roomCode, nickname) => new Promise((resolve) => {
      const sock = ensureSocket();
    ...
    updateConfig: (selectedRoles, discussionSeconds, nightStepSeconds) => new Promise((resolve) => {
      socketRef.current?.emit('update_config', { selectedRoles, discussionSeconds, nightStepSeconds }, (res) => {
        if (res && !res.ok) dispatch({ type: 'set_error', message: res.error });
        resolve(res);
      });
    }),
    startGame: () => new Promise((resolve) => {
      socketRef.current?.emit('start_game', null, (res) => {
        if (res && !res.ok) dispatch({ type: 'set_error', message: res.error });
        resolve(res);
      });
    }),
    nightAction: (payload) => new Promise((resolve) => {
